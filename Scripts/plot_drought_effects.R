...
p1 <- ggplot(treatment, aes(x = estimate_mean, y = VI , colour = contains_zero)) +
  geom_point() +
...
p2 <-  ggplot(treatment_clone, aes(x = estimate_mean, y = model_term , colour = contains_zero)) +
    geom_point() +
 ...
 p3 <-  ggplot(non_clone, aes(x = estimate_mean, y = model_term , colour = contains_zero)) +
    geom_point() +
    facet_wrap(~VI, scales = "free_x",) +
    geom_errorbarh(aes(xmin = lower, xmax = upper)) # +
  xlim(-.5, .5)
